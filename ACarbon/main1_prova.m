...
B=[1.35, 0.2];  %The constant related to the boundary condition (B.C) mass transfer. (small B, absorb slowly, big B, faster absortion)
...
for i = 1:length(Dp)
    for j = 1:length(B)
        ...
        figure(k)
        plot(ts,[qR,qb]), legend('qr','qb'),xlabel('t'), title("Dp = " + Dp(i) + "B = " + B(j))
        ...
        figure(k+1)
        plot(ts,[qRU,qbU]), legend('qr','qb'),xlabel('t'), title("Dp = " + Dp(i) + "B = " + B(j))
        
        ts = [0:nOfTimeSteps + nOfTimeStepsUnloading+1]*dt;
        figure(k+2)
        qRT = [qRLoading;qRU];
        qbT = [qbLoading;qbU];
        plot(ts,[qRT,qbT]), legend('qr', 'qb'),xlabel('t'), title("Dp = " + Dp(i) + "B = " + B(j))
        k = k+3;
